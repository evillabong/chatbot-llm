...
- **Flujos de trabajo (◐ motor base hecho):** motor de reglas "evento → condición → acción" (enrutar,
  ...
  sobre campos del evento (operadores: igual/distinto/contiene/no contiene), acciones **crear tarea**
  (título con marcadores `{campo}`) y **escalar a funcionario** (crea ticket y lo encola); endpoints
  `/automation-rules` (TenantAdmin) y UI `/automatizaciones`. **Pendiente:** más acciones
  (etiquetar/enrutar/disparar encuesta).

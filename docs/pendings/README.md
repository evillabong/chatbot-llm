...
| 20 | **Consola Fase C corte 2: mensajería en vivo (SignalR)** | 🔴 | Enviar/recibir mensajes en vivo (TicketHub `SendMessageToCitizen` + ChatHub `MessageReceived`), notificación de cola (`TicketEnqueued`/`TicketAssigned`/`TicketResolved`), transferencias, chat interno y encuesta. Usar `Microsoft.AspNetCore.SignalR.Client` con el token por `access_token`. Depende de CORS para SignalR (#5). |

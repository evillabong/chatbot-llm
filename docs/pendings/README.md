...
| 20 | **Consola Fase C corte 3: cola en vivo, transferencias, chat interno, encuesta** | 🟡 | Hecho el corte 2 (mensajería en vivo enviar/recibir). Falta: notificación de cola (`TicketEnqueued`/`TicketAssigned`/`TicketResolved`, requiere IDs de rol del agente — exponer endpoint o claim de roles), transferencias (`/tickets/transfer`), chat interno (`/internal-chat`) y encuesta. |

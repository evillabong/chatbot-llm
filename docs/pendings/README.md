...
| 15 | **CRUD de roles (`/roles`) en `Mimo.App`** | 🟡 | Cierra la Fase A. Reutiliza `MimoTable`/`MimoModal`/`MimoFormGroup` y un `RolesService` con escritura (hoy `RolesService` solo lee). Campos: nombre, descripción, prioridad, `CanViewAllTickets`, activo. |

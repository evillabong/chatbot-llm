...
| 20 | **Consola Fase C: cola en vivo, chat interno, encuesta** | 🟡 | Hechos: mensajería en vivo (corte 2) y transferencias. Falta: notificación de cola en vivo (`TicketEnqueued`/`TicketAssigned`/`TicketResolved`, requiere IDs de rol del agente — añadir claim `role_id` al JWT), chat interno (`/internal-chat`) y encuesta. |
...
| 22 | **Mejora continua: el asistente aprende con el uso (plus premium)** | 🟢 | Capacidad NO-core, gobernada por plan y **por organización (datos aislados, nunca cruzados)**. Objetivo: respuestas más precisas y derivación IA→funcionario más inteligente con el flujo diario. **No reentrenar el LLM**; usar bucles alrededor del LLM. Fases: (1) sugerir conocimiento desde conversaciones resueltas/calificaciones (con aprobación humana); (2) clasificador de derivación (resuelto-por-bot vs escalado vs reabierto, sobre el historial); (3) modelo especializado por tenant solo si el volumen/ROI lo justifican. **Captura ya lista** (transcripciones, ciclo de ticket = resuelto con/sin funcionario, motivo de escalamiento, encuestas); no requiere reestructurar el core hoy. |
